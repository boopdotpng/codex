...
impl ToolExecutor<ToolInvocation> for UpdateGoalHandler {
    ...
    async fn handle(
        ...
        if !matches!(
            args.status,
            ThreadGoalStatus::Complete | ThreadGoalStatus::Paused
        ) {
            return Err(FunctionCallError::RespondToModel(
                "update_goal can only mark the existing goal complete or paused; resume and budget-limited status changes are controlled by the user or system"
                    .to_string(),
            ));
        }
        session
        ...
        let goal = session
            ...
            .await
            .map_err(|err| FunctionCallError::RespondToModel(format_goal_error(err)))?;
        let report_mode = if args.status == ThreadGoalStatus::Complete {
            CompletionBudgetReport::Include
        } else {
            CompletionBudgetReport::Omit
        };
        goal_response(Some(goal), report_mode).map(boxed_tool_output)
    }
}
